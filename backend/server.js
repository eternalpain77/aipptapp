...
【重要约束】你必须严格围绕用户给定的主题生成内容，禁止自行替换、改名或转移到其他主题。整份 PPT 的标题与各页标题都要紧扣该主题，不得出现无关主题词。
...
- title: 整个演示的标题（必须直接体现用户主题，例如「{主题}」或「{主题}：副标题」，不得改成无关主题）
- slides: 每一页的数组，每页包含：
  - title: 页标题（必须围绕用户主题展开）
    ...
    const response = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "你是一个专业的 PPT 结构和文案生成助手。你绝对严格遵循用户给定的主题，绝不偏离或替换主题。输出只用 JSON。"
          },
          {
            role: "user",
            content: userPrompt
